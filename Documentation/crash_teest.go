...
func main() {
  // save bcd
  rawBCD, err := ioutil.ReadFile("course_data_036.bcd")
  if err != nil {
    log.Fatal(err)
  }
  level, err := LoadBCD(rawBCD)
  ...
  //Change K for multi-batch testing (Piranha Creepers and such)
  k := uint32(0x1)
  // Port Course to 3DW
  //level.Header.GameStyle = [2]byte{0x33,0x57}
  // Port Course to 2DW
  level.Header.GameStyle = [2]byte{0x57,0x55}
  for i := 0; i < 1000; i++ {
  //Run only when you need it. Can also change to be "^= 0x4" or similar to see interactions between flags
  //level.OverWorld.Objects[i].Flag = 0x6000040 ^ k
  //Used for if you need to put the objects inside ? blocks or similar. 
  //level.OverWorld.Objects[i].CId = 115
  //level.OverWorld.Objects[i].CFlag = 0x6000040
  level.OverWorld.Objects[i].Id = 132
  k = k << 1
  }
  spew.Dump(level.OverWorld.Objects[0].Flag)
  // save bcd
  newBCD, err := level.Save()
  if err != nil {
    log.Fatal(err)
  ...
  err = ioutil.WriteFile("course_data_036.bcd", newBCD, 0600)}
