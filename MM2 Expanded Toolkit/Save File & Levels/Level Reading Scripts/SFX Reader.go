...
func main() {
  ...
  if err != nil {
	...
	fmt.Println("OverWorld Sound Effect: ", int(level.OverWorld.SoundEffectCount))
    headerFmt := color.New(color.FgGreen, color.Underline).SprintfFunc()
    columnFmt := color.New(color.FgYellow).SprintfFunc()
    //Id: The index of the SFX, 0 to 55. A handful of unused ones, and 22 is Oshabi Drunkard.
	//X-Pos (X): X position of the SFX in 16ths of a block (I think?)
	//Y-Pos (Y): Y position of the SFX in 16ths of a block (I think?)
	//Unk1: Unknown
	tbl := table.New("Index", 
	"Id", 
	"X-Pos", 
    ...
    for i := 0; i < int(level.OverWorld.SoundEffectCount); i++ {
		tbl.AddRow(i, 
		level.OverWorld.Sounds[i].Id,
		level.OverWorld.Sounds[i].X, 
		level.OverWorld.Sounds[i].Y,
		level.OverWorld.Sounds[i].Unk1)
		}
	tbl.Print()
	fmt.Println("SubWorld Sound Effect: ", int(level.SubWorld.SoundEffectCount))
	tbl = table.New("Index", 
	"Id", 
	"X-Pos", 
	"Y-Pos",
	"Unk1")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)
    for i := 0; i < int(level.SubWorld.SoundEffectCount); i++ {
		tbl.AddRow(i, 
		level.SubWorld.Sounds[i].Id,
		level.SubWorld.Sounds[i].X, 
		level.SubWorld.Sounds[i].Y,
		level.SubWorld.Sounds[i].Unk1)
		}
    tbl.Print()
