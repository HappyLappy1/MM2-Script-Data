...
func main() {
  ...
  rawBCD, err := ioutil.ReadFile("course_data_028.bcd")
  if err != nil {
    log.Fatal(err)
  }
  ...
  if err != nil {
	...
	fmt.Println("OverWorld Tracks: ", level.OverWorld.TrackCount)
    headerFmt := color.New(color.FgGreen, color.Underline).SprintfFunc()
    columnFmt := color.New(color.FgYellow).SprintfFunc()
    //X-Pos (X): X position of the Track in full blocks (I think?)
	//Y-Pos (Y): Y position of the Track in full blocks (I think?)
	// flags: may relate to the things on the tracks, unsure how this works...
	// type: Track Type, Curved, Switch, Orientation, etc.
	// LID: What radio frequency is this track set to? other objects on this frequency will act as if on these tracks. 
	// unk1: We don't know: Usually 0
	// unk2: We don't know: common values seem to be 112, 113, 144, 135, 260, etc. Leads me to suspect a bitfield. Rarely hits ~55,000.
	// unk3: We don't know: common values seem to be 112, 113, 144, 135, 260, etc. Leads me to suspect a bitfield. Rarely hits ~8,100. 
	tbl := table.New("Index", "X-Pos", "Y-Pos", "Flags", "Type", "Linked_Id", "Unk1", "Unk2", "Unk3","Background_Id")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)
    for i := 0; i < int(level.OverWorld.TrackCount); i++ {
		tbl.AddRow(i, 
		level.OverWorld.Tracks[i].X, 
		level.OverWorld.Tracks[i].Y,
		level.OverWorld.Tracks[i].Flags,
		level.OverWorld.Tracks[i].Type,
		level.OverWorld.Tracks[i].LId,
		level.OverWorld.Tracks[i].Unk1,
		level.OverWorld.Tracks[i].Unk2,
		level.OverWorld.Tracks[i].Unk3)
		}
	tbl.Print()
	fmt.Println("SubWorld Tracks: ", level.SubWorld.TrackCount)
	tbl = table.New("Index", "X-Pos", "Y-Pos", "Flags", "Type", "Linked_Id", "Unk1", "Unk2", "Unk3","Background_Id")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)
    for i := 0; i < int(level.SubWorld.TrackCount); i++ {
		tbl.AddRow(i, 
		level.SubWorld.Tracks[i].X, 
		level.SubWorld.Tracks[i].Y,
		level.SubWorld.Tracks[i].Flags,
		level.SubWorld.Tracks[i].Type,
		level.SubWorld.Tracks[i].LId,
		level.SubWorld.Tracks[i].Unk1,
		level.SubWorld.Tracks[i].Unk2,
		level.SubWorld.Tracks[i].Unk3)
    }
	tbl.Print()	
  // save bcd
  newBCD, err := level.Save()
