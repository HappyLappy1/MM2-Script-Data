...
func main() {
  // save bcd
  // Choose which level file to read (such as "course_data_036.bcd")
  rawBCD, err := ioutil.ReadFile("course_data_037.bcd")
  if err != nil {
    log.Fatal(err)
  }
  ...
  if err != nil {
	...
	fmt.Println("Overworld Objects: ", int(level.OverWorld.ObjectCount))
    headerFmt := color.New(color.FgGreen, color.Underline).SprintfFunc()
    columnFmt := color.New(color.FgYellow).SprintfFunc()
    //X-Pos (X): X position of the object in 16ths of a block (I think?)
	//Y-Pos (Y): Y position of the object in 16ths of a block (I think?)
	//Unk1: Unknown
	//Width: How wide is the object in blocks?
	//Height: How tall is the object in blocks?  
	//Flag: Flag affects a lot of stuff. Size, color/variant, wings, parachute, etc.
	//Contents Flag (CFlag): Flag, but for the contents
	//Extension (EX): Seems to apply to vines, perhaps how they're connected to ground?
	//Object_Id: The Id of the object. See Toost for a full list
	//Contents_Id: The Id of an object contained inside the object (such as a ? block)
	//Link Id: Another object this object is "linked" to. Pipes link to what they dispense, Claws link to what they hold, Tracks link to what they dispense, etc. 
	//Sound_Id (SId): The Id of the sound effect applied to the object
	tbl := table.New("Index", 
	"X-Pos", 
	"Y-Pos",
	"Unk1",
    ...
    for i := 0; i < int(level.OverWorld.ObjectCount); i++ {
		tbl.AddRow(i, 
		level.OverWorld.Objects[i].X, 
		level.OverWorld.Objects[i].Y,
		level.OverWorld.Objects[i].Unk1,
		level.OverWorld.Objects[i].Width,
		level.OverWorld.Objects[i].Height,
		level.OverWorld.Objects[i].Flag,
		level.OverWorld.Objects[i].CFlag,
		level.OverWorld.Objects[i].Ex,
		level.OverWorld.Objects[i].Id,
		level.OverWorld.Objects[i].CId,
		level.OverWorld.Objects[i].LId,
		level.OverWorld.Objects[i].SId)
		}
	tbl.Print()
	fmt.Println("Subworld Objects: ", int(level.SubWorld.ObjectCount))
	tbl = table.New("Index", 
	"X-Pos", 
	"Y-Pos",
	"Unk1",
	...
	"Sound_Id")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)
    for i := 0; i < int(level.SubWorld.ObjectCount); i++ {
		tbl.AddRow(i, 
		level.SubWorld.Objects[i].X, 
		level.SubWorld.Objects[i].Y,
		level.SubWorld.Objects[i].Unk1,
		level.SubWorld.Objects[i].Width,
		level.SubWorld.Objects[i].Height,
		level.SubWorld.Objects[i].Flag,
		level.SubWorld.Objects[i].CFlag,
		level.SubWorld.Objects[i].Ex,
		level.SubWorld.Objects[i].Id,
		level.SubWorld.Objects[i].CId,
		level.SubWorld.Objects[i].LId,
		level.SubWorld.Objects[i].SId)
		}
    tbl.Print()
  // save bcd
  newBCD, err := level.Save()
