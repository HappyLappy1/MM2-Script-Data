...
func test_roundtrip() error {
	...
	buf, err := ioutil.ReadFile("course_data_018.bcd")
	if err != nil {
		return err
	}
...
func main() {
  ...
  rawBCD, err := ioutil.ReadFile("course_data_015.bcd")
  if err != nil {
    log.Fatal(err)
  }
  ...
  if err != nil {
	...
	fmt.Println("OverWorld Snake Blocks: ", level.OverWorld.SnakeBlockCount)
	fmt.Println("OverWorld Clear Pipes: ", level.OverWorld.ClearPipeCount)
	fmt.Println("OverWorld Track Blocks: ", level.OverWorld.TrackBlockCount)
	fmt.Println("OverWorld ! Blocks: ", level.OverWorld.ExclamationMarkBlockCount)
	fmt.Println("OverWorld Piranha Creepers: ", level.OverWorld.PiranhaCreeperCount)
	...
	//Unk1: Unknown, but likely determines the orientation of Snakeblocks, where the nodes protrude, etc.
	//Nodes are the block-by-block for where the snake-block travels. 
	//Node Direction: 0 = Pause, 1 = Left, 2 = Right, 3 = Down, 4 = Up, 5 = Left -> Down, 6 = Down -> Left, 7 = Left -> Up, 8 = Up -> Left, 9 = Right -> Down, 10 = Down -> Right, 11 = Right -> Up, 12 = Up -> Right, 13 = Direction -> Stop
	tbl := table.New("Index", "Object Type", "Unk1", "# of Nodes", "Path Taken")
	...
	car_dir := ""
	temp := 0
	for i := 0; i < int(level.OverWorld.SnakeBlockCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.OverWorld.Snakes[i].NodeCount); j++ {
			if int(level.OverWorld.Snakes[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.OverWorld.Snakes[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.OverWorld.Snakes[i].Nodes[j].Direction) > 4{
				temp = int(level.OverWorld.Snakes[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.OverWorld.Snakes[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
		...
		tbl.AddRow(i,"Snake_Block", level.OverWorld.Snakes[i].Unk1,level.OverWorld.Snakes[i].NodeCount, path)
	}
	for i := 0; i < int(level.OverWorld.ClearPipeCount); i++ {
		path = ""
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.OverWorld.ClearPipes[i].NodeCount); j++ {
			if int(level.OverWorld.ClearPipes[i].Nodes[j].Direction) == 0{
				if int(level.OverWorld.ClearPipes[i].Nodes[j].Height) > 0{
				car_dir = "Rx"
				cur_len = int(level.OverWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "}
			} else if int(level.OverWorld.ClearPipes[i].Nodes[j].Direction) == 2{
				car_dir = "Ux"
				cur_len = int(level.OverWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else if int(level.OverWorld.ClearPipes[i].Nodes[j].Direction) == 1{
				car_dir = "Lx"
				cur_len = int(level.OverWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else if int(level.OverWorld.ClearPipes[i].Nodes[j].Direction) == 3{
				car_dir = "Dx"
				cur_len = int(level.OverWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else {
		...
		path = path + "P"
		tbl.AddRow(i,"Clear_Pipes", level.OverWorld.ClearPipes[i].Unk,level.OverWorld.ClearPipes[i].NodeCount, path)
	}
	for i := 0; i < int(level.OverWorld.TrackBlockCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.OverWorld.TrackBlocks[i].NodeCount); j++ {
			if int(level.OverWorld.TrackBlocks[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.OverWorld.TrackBlocks[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.OverWorld.TrackBlocks[i].Nodes[j].Direction) > 4{
				temp = int(level.OverWorld.TrackBlocks[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.OverWorld.TrackBlocks[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Track_Block", level.OverWorld.TrackBlocks[i].Unk1,level.OverWorld.TrackBlocks[i].NodeCount, path)
	}
	for i := 0; i < int(level.OverWorld.ExclamationMarkBlockCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.OverWorld.ExclamationBlocks[i].NodeCount); j++ {
			if int(level.OverWorld.ExclamationBlocks[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.OverWorld.ExclamationBlocks[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.OverWorld.ExclamationBlocks[i].Nodes[j].Direction) > 4{
				temp = int(level.OverWorld.ExclamationBlocks[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.OverWorld.ExclamationBlocks[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Exclamation_Block", level.OverWorld.ExclamationBlocks[i].Unk1,level.OverWorld.ExclamationBlocks[i].NodeCount, path)
	}
	for i := 0; i < int(level.OverWorld.PiranhaCreeperCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.OverWorld.PiranhaCreepers[i].NodeCount); j++ {
			if int(level.OverWorld.PiranhaCreepers[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.OverWorld.PiranhaCreepers[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.OverWorld.PiranhaCreepers[i].Nodes[j].Direction) > 4{
				temp = int(level.OverWorld.PiranhaCreepers[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.OverWorld.PiranhaCreepers[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Piranha_Creeper", level.OverWorld.PiranhaCreepers[i].Unk1,level.OverWorld.PiranhaCreepers[i].NodeCount, path)
	}
	
	tbl.Print()

	fmt.Println("SubWorld Snake Blocks: ", level.SubWorld.SnakeBlockCount)
	fmt.Println("SubWorld Clear Pipes: ", level.SubWorld.ClearPipeCount)
	fmt.Println("SubWorld Track Blocks: ", level.SubWorld.TrackBlockCount)
	fmt.Println("SubWorld ! Blocks: ", level.SubWorld.ExclamationMarkBlockCount)
	fmt.Println("SubWorld Piranha Creepers: ", level.SubWorld.PiranhaCreeperCount)
    headerFmt = color.New(color.FgGreen, color.Underline).SprintfFunc()
    columnFmt = color.New(color.FgYellow).SprintfFunc()
	tbl = table.New("Index", "Object Type", "Unk1", "# of Nodes", "Path Taken")
	tbl.WithHeaderFormatter(headerFmt).WithFirstColumnFormatter(columnFmt)
	...
	car_dir = "S"
	temp = 0
	for i := 0; i < int(level.SubWorld.SnakeBlockCount); i++ {
		path = ""
		cur_len = 0
		cur_dir = 0
		car_dir = ""
		for j := 0; j < int(level.SubWorld.Snakes[i].NodeCount); j++ {
			if int(level.SubWorld.Snakes[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.SubWorld.Snakes[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.SubWorld.Snakes[i].Nodes[j].Direction) > 4{
				temp = int(level.SubWorld.Snakes[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.SubWorld.Snakes[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Snake_Block", level.SubWorld.Snakes[i].Unk1,level.SubWorld.Snakes[i].NodeCount, path)
	}
	for i := 0; i < int(level.SubWorld.ClearPipeCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.SubWorld.ClearPipes[i].NodeCount); j++ {
			if int(level.SubWorld.ClearPipes[i].Nodes[j].Direction) == 0{
				if int(level.SubWorld.ClearPipes[i].Nodes[j].Height) > 0{
				car_dir = "Rx"
				cur_len = int(level.SubWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "}
			} else if int(level.SubWorld.ClearPipes[i].Nodes[j].Direction) == 2{
				car_dir = "Ux"
				cur_len = int(level.SubWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else if int(level.SubWorld.ClearPipes[i].Nodes[j].Direction) == 1{
				car_dir = "Lx"
				cur_len = int(level.SubWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else if int(level.SubWorld.ClearPipes[i].Nodes[j].Direction) == 3{
				car_dir = "Dx"
				cur_len = int(level.SubWorld.ClearPipes[i].Nodes[j].Height)
				path = path + car_dir + strconv.Itoa(cur_len) + ", "
			} else {
		...
		path = path + "P"
		tbl.AddRow(i,"Clear_Pipes", level.SubWorld.ClearPipes[i].Unk,level.SubWorld.ClearPipes[i].NodeCount, path)
	}
		for i := 0; i < int(level.SubWorld.TrackBlockCount); i++ {
		path = ""
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.SubWorld.TrackBlocks[i].NodeCount); j++ {
			if int(level.SubWorld.TrackBlocks[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.SubWorld.TrackBlocks[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.SubWorld.TrackBlocks[i].Nodes[j].Direction) > 4{
				temp = int(level.SubWorld.TrackBlocks[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.SubWorld.TrackBlocks[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Track_Block", level.SubWorld.TrackBlocks[i].Unk1,level.SubWorld.TrackBlocks[i].NodeCount, path)
	}
	for i := 0; i < int(level.SubWorld.ExclamationMarkBlockCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.SubWorld.ExclamationBlocks[i].NodeCount); j++ {
			if int(level.SubWorld.ExclamationBlocks[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.SubWorld.ExclamationBlocks[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.SubWorld.ExclamationBlocks[i].Nodes[j].Direction) > 4{
				temp = int(level.SubWorld.ExclamationBlocks[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.SubWorld.ExclamationBlocks[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Exclamation_Block", level.SubWorld.ExclamationBlocks[i].Unk1,level.SubWorld.ExclamationBlocks[i].NodeCount, path)
	}
	for i := 0; i < int(level.SubWorld.PiranhaCreeperCount); i++ {
		path = "Sx"
		cur_len = 1
		cur_dir = 0
		car_dir = "S"
		for j := 0; j < int(level.SubWorld.PiranhaCreepers[i].NodeCount); j++ {
			if int(level.SubWorld.PiranhaCreepers[i].Nodes[j].Direction) == cur_dir{
				cur_len = cur_len + 1
			} else if int(level.SubWorld.PiranhaCreepers[i].Nodes[j].Direction) == 13{
				car_dir = "P"
				path = path + strconv.Itoa(cur_len) + ", " + car_dir
				cur_len = 1
			} else if int(level.SubWorld.PiranhaCreepers[i].Nodes[j].Direction) > 4{
				temp = int(level.SubWorld.PiranhaCreepers[i].Nodes[j].Direction)
				if temp == 6 {
				cur_dir = 1
				car_dir = "Lx"
				}else if temp == 8 {
				...
				cur_dir = int(level.SubWorld.PiranhaCreepers[i].Nodes[j].Direction)
				if cur_dir == 1 {
				car_dir = "Lx"
				} else if cur_dir == 3 {
				car_dir = "Dx"
	...
	tbl.AddRow(i,"Piranha_Creeper", level.SubWorld.PiranhaCreepers[i].Unk1,level.SubWorld.PiranhaCreepers[i].NodeCount, path)
	}
    tbl.Print()
  // save bcd
  newBCD, err := level.Save()
