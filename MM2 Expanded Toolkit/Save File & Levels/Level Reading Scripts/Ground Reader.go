...
func main() {
  ...
  if err != nil {
	...
	fmt.Println("OverWorld Ground: ", level.OverWorld.GroundCount)
    headerFmt := color.New(color.FgGreen, color.Underline).SprintfFunc()
    columnFmt := color.New(color.FgYellow).SprintfFunc()
    //X-Pos (X): X position of the ground in full blocks (I think?)
	//Y-Pos (Y): Y position of the ground in full blocks (I think?)
	//background_id: What type of ground tile is it? Does it have decoration? (I guess?)
	tbl := table.New("Index", 
	"X-Pos", 
	"Y-Pos",
	"Background_Id")
	...
	tbl.Print()
	fmt.Println("SubWorld Ground: ", level.SubWorld.GroundCount)
	tbl = table.New("Index", 
	"X-Pos", 
	"Y-Pos",
	"Background_Id")
