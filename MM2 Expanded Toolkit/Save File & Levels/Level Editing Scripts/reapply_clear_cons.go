package main
// Start reading at line 573 (I don't understand anything before that point)
import (
	"bytes"
	"crypto/aes"
	"crypto/cipher"
...
func main() {
  // Select the bcd file you wish to edit (in this case, "course_data_000.bcd")
  rawBCD, err := ioutil.ReadFile("course_data_016.bcd")
  if err != nil {
    log.Fatal(err)
  }
  // Load that file, whatever that means
  level, err := LoadBCD(rawBCD)
  if err != nil {
    log.Fatal(err)
  }
  
  // Begin Editing the level file here!

  // Edit Timer (Integer between 10 and 500 will upload. Feel free to experiment!)
	level.Header.TimeLimit = 500
  // Edit Clear Condition
  // Copy below lines for clear con of "Collect all 5 Toads"
  fmt.Println("Hi")
  level.Header.ClearConditionCategory = 1
  level.Header.ClearConditionObject = 4009912959
  level.Header.ClearConditionMagnitude = 5 // Or whatever value you choose
  
  // Saving and Exporting Modified Course 002
  newBCD, err := level.Save()
  if err != nil {
    log.Fatal(err)
  }
  // Name new output file "course_data_000.bcd" (replacing the old one. Will make a new file if name doesn't exist)
    err = ioutil.WriteFile("course_data_016.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
  
  
  // Select the bcd file you wish to edit (in this case, "course_data_001.bcd")
  rawBCD, err = ioutil.ReadFile("course_data_001.bcd")
  if err != nil {
    log.Fatal(err)
  }
  // Load that file, whatever that means
  ...
  // Edit Timer (Integer between 10 and 500 will upload. Feel free to experiment!)
	level.Header.TimeLimit = 500
  // Edit Clear Condition
  // Copy below lines for clear con of "Carry Stone to Goal"
  level.Header.ClearConditionMagnitude = 0
  level.Header.ClearConditionCategory = 2
  level.Header.ClearConditionObject = 1176604998
  ...
  // Name new output file "course_data_001.bcd" (replacing the old one. Will make a new file if name doesn't exist)
    err = ioutil.WriteFile("course_data_001.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
  ...
  // Select the bcd file you wish to edit (in this case, "course_data_002.bcd")
  rawBCD, err = ioutil.ReadFile("course_data_002.bcd")
  if err != nil {
    log.Fatal(err)
  }
  // Load that file, whatever that means
  ...
  // Edit Timer (Integer between 10 and 500 will upload. Feel free to experiment!)
	level.Header.TimeLimit = 500
  // Edit Clear Condition
  // Copy below lines for clear con of "Don't Exit Water"
  level.Header.ClearConditionMagnitude = 0
  level.Header.ClearConditionCategory = 3
  level.Header.ClearConditionObject = 988953566
  ...
  // Name new output file "course_data_002.bcd" (replacing the old one. Will make a new file if name doesn't exist)
    err = ioutil.WriteFile("course_data_002.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
  
  // Select the bcd file you wish to edit (in this case, "course_data_003.bcd")
  rawBCD, err = ioutil.ReadFile("course_data_003.bcd")
  if err != nil {
    log.Fatal(err)
  }
  // Load that file, whatever that means
  ...
  // Edit Timer (Integer between 10 and 500 will upload. Feel free to experiment!)
	level.Header.TimeLimit = 500
  // Edit Clear Condition
  // Copy below lines for clear con of "Don't Touch Claws"
  level.Header.ClearConditionMagnitude = 0
  level.Header.ClearConditionCategory = 3
  level.Header.ClearConditionObject = 2549403514

  // Saving and Exporting Modified Course 003
  newBCD, err = level.Save()
  if err != nil {
    log.Fatal(err)
  }
  // Name new output file "course_data_003.bcd" (replacing the old one. Will make a new file if name doesn't exist)
    err = ioutil.WriteFile("course_data_003.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
  
  
  // Select the bcd file you wish to edit (in this case, "course_data_004.bcd")
  rawBCD, err = ioutil.ReadFile("course_data_004.bcd")
  if err != nil {
    log.Fatal(err)
  }
  // Load that file, whatever that means
  ...
  // Edit Timer (Integer between 10 and 500 will upload. Feel free to experiment!)
	level.Header.TimeLimit = 500
  // Edit Clear Condition
  // Copy below lines for clear con of "Climb 3 Trees"
  level.Header.ClearConditionMagnitude = 3 // or whatever value you want
  level.Header.ClearConditionCategory = 1
  level.Header.ClearConditionObject = 3265790504

  // Saving and Exporting Modified Course 004
  newBCD, err = level.Save()
  if err != nil {
    log.Fatal(err)
  }
  // Name new output file "course_data_004.bcd" (replacing the old one. Will make a new file if name doesn't exist)
    err = ioutil.WriteFile("course_data_004.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
}