...
func main() {
  ...
  // Copy below lines for clear con of "Collect all 6 Toads"
  level.Header.ClearConditionCategory = 1
  level.Header.ClearConditionObject = 4009912959
  level.Header.ClearConditionMagnitude = 6 // Or whatever value you choose

  //Make Overworld Vertical (Does funky things if goal is too far away: Horizontal is 0)
  //level.OverWorld.Orientation = 1
  
  // Set First SFX in the OverWorld to unused SFX "Oshabi Drunkard" (line below)
  level.OverWorld.Sounds[0].Id = 22
  
  // Set First 7 objects in the OverWorld to exclusive stuff: ID # 75 is for stone, ID # 73 is for toad.
  // Make Stone (Copy below line to another BCD to replace object 0 with a stone)
  level.OverWorld.Objects[0].Id = 75  
  // Flags determine toad color
  // Make Toad (Copy below lines to another BCD to replace object 1 with a yellow-hat toad)
  level.OverWorld.Objects[1].Id = 73
  // Make Toad Hat Yellow
  level.OverWorld.Objects[1].Flag = 117440576
  // Make Toad (Copy below lines to another BCD to replace object 2 with a red-hat toad)
  level.OverWorld.Objects[2].Id = 73
  // Make Toad Hat Red
  level.OverWorld.Objects[2].Flag = 104857664
  // Make Toad (Copy below lines to another BCD to replace object 3 with a blue-hat toad)
  level.OverWorld.Objects[3].Id = 73
  // Make Toad Hat Blue
  level.OverWorld.Objects[3].Flag = 109051968
  // Make Toad (Copy below lines to another BCD to replace object 4 with a green-hat toad)
  level.OverWorld.Objects[4].Id = 73
  // Make Toad Hat Green
  level.OverWorld.Objects[4].Flag = 113246272
  // Make Toad (Copy below lines to another BCD to replace object 5 with a purple-hat toad)
  level.OverWorld.Objects[5].Id = 73
  // Make Toad Hat Purple 
  level.OverWorld.Objects[5].Flag = 121634880
  // Make Toad (Copy below lines to another BCD to replace object 6 with a hatless toad)
  level.OverWorld.Objects[6].Id = 73
  // Make Toad Hatless
  level.OverWorld.Objects[6].Flag = 100663360
  // Saving and Exporting Modified Course 000
  newBCD, err := level.Save()
  if err != nil {
    log.Fatal(err)
  }
// Name new output file "course_data_000.bcd" (replacing the old one. Will make a new file if name doesn't exist)  
   err = ioutil.WriteFile("course_data_000.bcd", newBCD, 0600)
  if err != nil {
    log.Fatal(err)
  }
}