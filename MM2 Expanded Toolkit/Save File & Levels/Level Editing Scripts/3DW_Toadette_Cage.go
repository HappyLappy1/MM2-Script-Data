...
func main() {
  ...
  // Set Gamestyle to 3DW (Copy below line)
  level.Header.GameStyle = [2]byte{0x33,0x57}
  // Revert Courses to 2DW (Replace above values with "0x57,0x55")
  ...
  //Make Overworld Vertical (Does funky things if goal is too far away)
  ...
  // Set Objects 0 and 2 in the level to exclusive stuff: ID # 75 is for stone, ID # 115 is for toadette cage.
  // Make Stone (Copy below line to another BCD to replace object 0 with a stone)
  level.OverWorld.Objects[0].Id = 75  
  // Toadette Cage (Copy below line to another BCD to replace object 2 with a Toadette Cage)
  level.OverWorld.Objects[2].Id = 115
  // Saving and Exporting Modified Course 001
  newBCD, err := level.Save()
  if err != nil {
    log.Fatal(err)
  }
